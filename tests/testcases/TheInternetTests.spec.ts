...
import { isCheckboxChecked, toggleCheckbox } from '../pages/CheckboxPage';
import { checkThatKeyPressInputIsDisplayed, getLastKeyPressed, sendKey } from '../pages/KeyPressPage';
...
test.describe('The Internet App Tests', () => {
  ...
  test('Key presses test', async () => {
    ...
    console.assert(
      resultW?.includes('You entered: W'),
      `Key press failed: Expected "You entered: W", but got "${resultW}"`,
    );
    ...
    console.assert(
      resultA?.includes('You entered: A'),
      `Key press failed: Expected "You entered: A", but got "${resultA}"`,
    );
